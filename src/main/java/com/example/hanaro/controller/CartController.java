...
public class CartController {
	...
	public ResponseEntity<String> addCartItem(@RequestBody CartItemAddDto cartItemAddDto, Authentication authentication) {
			String name = authentication.getName();
			log.info("ddddddddddd: {}", name);
		try {
			cartService.findCartOrMake(name);
			cartItemsService.addCartItem(name,cartItemAddDto);
			log.info("장바구니에 상품 추가 성공: {}", cartItemAddDto.getItemName());
	...
	public ResponseEntity<String> deleteCartItem(@RequestBody CartItemDeleteDto deleteDto, Authentication authentication) {
		try {
			String name = authentication.getName();
			cartItemsService.deleteCartItem(name, deleteDto);
			log.info("장바구니 상품 삭제 성공: itemId={}", deleteDto.getName());
...
//
	...
	public ResponseEntity<List<CartItemResponseDto>> getCartItems(Authentication authentication) {
		try {
			String name = authentication.getName();
			List<CartItemResponseDto> cartItems = cartItemsService.getCartItems(name);
